...
# from rory.core.interfaces.securereplicator import SecureReplicator,DumbReplicator
from dotenv import load_dotenv
load_dotenv()

NODE_ID                       = os.environ.get("NODE_ID","scm-0")
IP_ADDR                          = os.environ.get("NODE_IP_ADDR","0.0.0.0")
PORT                          = int(os.environ.get("NODE_PORT",6000))
NODE_PREFIX                   = os.environ.get("NODE_PREFIX","scw-")
init_workers              = int(os.environ.get("INIT_WORKERS",1))
MAX_WORKERS               = int(os.environ.get("MAX_WORKERS",4))
DOCKER_IMAGE_NAME         = os.environ.get("DOCKER_IMAGE_NAME","secure-clustering-worker")
...
def create_app(*args):
    ...
    with app.app_context():
        ...
        # current_app.config["summoner"]                          = REPLICATOR
        
        current_app.config["defaults"]                         = {}
        current_app.config["defaults"]["NODE_PREFIX"]          = NODE_PREFIX
        current_app.config["defaults"]["DOCKER_IMAGE_NAME"]    = DOCKER_IMAGE_NAME
        current_app.config["defaults"]["DOCKER_IMAGE_TAG"]     = DOCKER_IMAGE_TAG
        current_app.config["defaults"]["DOCKER_IMAGE_VERSION"] = "{}:{}".format(DOCKER_IMAGE_NAME,DOCKER_IMAGE_TAG)
...
if __name__ == '__main__':
    app    = create_app()
    # if not TESTING: #Testing es falso no va a activar el replicador
    #     t1     = Thread(
    #         target = deploy_init_workers,
    #         daemon = True,
    #         args   = (INIT_WORKERS,NODE_ID,PORT,DOCKER_IMAGE,REPLICATOR,LOGGER,NODE_PREFIX,MAX_WORKERS,app,TESTING,STORAGE_SYSTEM_HOSTNAME,STORAGE_SYSTEM_PORT,MAX_RETRIES)
    #     )
    #     t1.start()
