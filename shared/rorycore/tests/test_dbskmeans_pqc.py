...
def test_init(ckks_client):
    ...
    dbs = DBSKMeans(scheme=ckks_client, init_shiftmatrix=init_shift)
    assert dbs.scheme is not None
    assert dbs.init_shiftmatrix is not None


def test_compute_centroid_shift(ckks_client):
    dbs = DBSKMeans(scheme=ckks_client, init_shiftmatrix=ckks_client.encryptVector(np.zeros(4, dtype=np.float32)))
