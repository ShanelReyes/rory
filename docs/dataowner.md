...
The **Rory Dataowner** acts as the **Data Owner (DO)** and primary security authority within the **Rory** platform, a specialized system for **Post-Quantum Privacy-Preserving Data Mining as a Service (PPDMaaS)**. 

Its main mission is to ensure that confidential dataset never leave the local environment in plain text. Using a hybrid cryptographic approach, combining homomorphic encryption (Liu, CKKS) for secure arithmetic operations and order-preserving encryption (FDHOPE) for secure comparisons, the dataowner enables distributed computing on remote nodes without compromising data privacy.

!!! info "Batch Automation"
    For running experiments in bulk, use the [**Dataowner**](/dataowner) module. It automates experiment execution by reading trace CSV files and submitting workloads to the Dataowner service concurrently.
...
The **Rory Dataowner** is configured primarily through environment variables. These can be managed via a `.env` file (located by default at `/rory/envs/.dataowner.env`) as long as the `RORY_DEBUG` variable is enabled.

#### Node & Network Configuration
Defines the dataowner's identity and its communication parameters with orchestrators and processing nodes.

| Variable | Description | Default Value |
|:---|:---|:---|
| `NODE_ID` | Unique identifier for the dataowner node. | `rory-dataowner-0` |
| `NODE_IP_ADDR` | IP address or hostname of the dataowner node. | `NODE_ID` |
| `NODE_PORT` | Listening port for the dataowner service. | `3000` |
...
| `ENV_FILE_PATH` | Path to the .env configuration file. | `/rory/envs/.dataowner.env` |

---

## Local Usage

To run the **Rory Dataowner** in a local environment, follow these steps to ensure all cryptographic and storage dependencies are correctly configured.
...
* **Mictlanx (CSS)**: The storage layer must be running (usually via Docker) as the Dataowner attempts to initialize an `AsyncStorageClient` on startup.
...
```bash
export DATAOWNER_PATH=/home/<user>/rory
```

### Environment Setup

1. **Clone the repository and enter the dataowner directory**:
...
4.  **Initialize the Environment File**:
    ...
    Create the `.env` file in the path defined by `ENV_FILE_PATH` (default: `/rory/envs/.dataowner.env`). Ensure `RORY_DEBUG=1` is set to allow the dataowner to load these configurations on startup.
...
```bash
# Using default paths for node: rory-dataowner-0
mkdir -p /rory/rory-dataowner-0/source
mkdir -p /rory/rory-dataowner-0/sink
mkdir -p /rory/rory-dataowner-0/log
mkdir -p /rory/keys
```
...
### Executing the Dataowner Service
...
2. **Run using Gunicorn:** Once the storage is ready, navigate back to the dataowner source and start the service:

    ```bash
    cd ../dataowner/src
    gunicorn --chdir $DATAOWNER_PATH/src --config $DATAOWNER_PATH/src/gunicorn_config.py main:app
    # Alternatively, you can use the provided startup script:
    # ../run.sh
    ```
...
Once the service is running, verify the node's status and its identified role (Dataowner) by performing a health check:
...
The Client includes a dedicated `Dockerfile` located in its root directory (`/dataowner/Dockerfile`). The container has been created using a multi-layer approach to improve performance and deployment. Below is a breakdown of the environment configuration:
...
To build the Dataowner image manually, you must navigate to the dataowner's directory. The build process uses the local context to package the source code and configuration:

```bash
# Navigate to the dataowner directory
cd /rory/dataowner

# Build the image using the local Dockerfile
docker build -t rory:dataowner -f Dockerfile .
```

`-t`: Defines the repository name and tag (e.g., `rory:dataowner`).
...
For a standardized deployment, the `build.sh` script automates the process by targeting the dataowner folder structure dynamically.
...
# The script targets the dataowner component folder specifically
...
```bash
# Build the dataowner image using default values (rory:dataowner)
./build.sh

# Build with a custom image name and project path
./build.sh /home/sreyes/rory shanelreyes/rory:dataowner-prod
```
...
services:
  rory-dataowner-0:
    ...
    volumes:
      ...
      - /rory/rory-dataowner-0/keys:/rory/keys
...
Verify that the Dataowner image and container are correctly initialized in the Docker engine:

```bash
# Check dataowner image
docker images | grep rory:dataowner

# Check running dataowner container
docker ps | grep rory-dataowner-0
```