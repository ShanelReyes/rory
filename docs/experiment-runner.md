...
The **Rory Dataowner** module is a batch experiment runner that automates the execution of data mining workloads across the Rory platform. Unlike the Dataowner, Manager, and Worker components, it is **not a Flask web service** — it is a CLI script that processes a trace CSV file and submits experiments concurrently to the **Dataowner** service via the `roryclient` library.

## Key Responsibilities

- **Trace Processing:** Reads a CSV file defining which algorithms to run, with what parameters, and how many iterations.
- **Concurrent Execution:** Uses a `ThreadPoolExecutor` to submit experiments to the Dataowner service in parallel, with configurable inter-arrival delays.
...
3. The `ALGORITHM` column determines which Dataowner endpoint is called (e.g., `KMEANS` → `dataowner.kmeans()`).
...
| `DATAOWNER_IP_ADDR` | IP address or hostname of the Dataowner service. | `localhost` |
| `RORY_DATAOWNER_PORT` | Port of the Dataowner service. | `3000` |
| `RORY_DATAOWNER_TIMEOUT` | HTTP timeout for Dataowner requests (seconds). | `120` |
...
| `DATAOWNER_TIMEOUT` | Global timeout for experiment execution (seconds). | `300` |
...
The trace is a CSV file where each row defines a single experiment configuration. The columns in the trace map directly to the HTTP headers sent to the Dataowner service's algorithm endpoints.
...
| Algorithm | Task Category | Dataowner Method |
|:---|:---|:---|
| `KMEANS` | Clustering | `dataowner.kmeans()` |
| `SKMEANS` | Clustering | `dataowner.skmeans()` |
| `DBSKMEANS` | Clustering | `dataowner.dbskmeans()` |
| `SKMEANSPQC` | Clustering | `dataowner.skmeans_pqc()` |
| `DBSKMEANSPQC` | Clustering | `dataowner.dbskmeans_pqc()` |
| `NNC` | Clustering | `dataowner.nnc()` |
| `DBSNNC` | Clustering | `dataowner.dbsnnc()` |
| `KNN` | Classification | `dataowner.knn()` |
| `SKNN` | Classification | `dataowner.sknn()` |
| `SKNNPQC` | Classification | `dataowner.sknn_pqc()` |
| `LOGISTICREGRESSION` | Machine Learning | `dataowner.logistic_regression()` |
| `PPLR` | Machine Learning | `dataowner.pplr()` |
...
services:
  rory-dataowner-0:
    ...
    environment:
      ...
      - TRACE_ID=KMEANS
      - DATAOWNER_IP_ADDR=rory-dataowner-0
    volumes:
