...
async def pplr_train():
    try:
        ...
        worker_headers = {
            ...
            "Scale": str(scale),
            "N-Features": str(n_features_train),
        ...
        jsonWorkerResponse        = worker_response.json()
        train1_encrypted_out_weights_id       = jsonWorkerResponse["encrypted_out_weights_id"]
        train1_encrypted_out_bias_id          = jsonWorkerResponse["encrypted_out_bias_id"]

        logger.debug({
            "train1_encrypted_out_weights_id"     : train1_encrypted_out_weights_id,
            "train1_encrypted_out_bias_id"        : train1_encrypted_out_bias_id, 
        })
...
async def pplr_predict():
    try:
        ...
        if encrypted_test_put_chunk.is_err:
            logger.error({
                "msg": "Failed to put encrypted test matrix in storage",
                "encrypted_matrix_test_id": encrypted_matrix_test_id
            })
            return Response(status=500, response="Failed to put encrypted matrix in storage.")

        logger.debug({
            "msg": "Test encrypted dataset put in SAD successfully"
        })
        ...
        plaintext_matrix_weights = np.zeros((1,n_features_test), dtype=np.float32)
        ...
        jsonWorkerResponse        = worker_response.json()
        predict1_encrypted_out_weights_id       = jsonWorkerResponse["encrypted_out_weights_id"]
        predict1_encrypted_out_bias_id          = jsonWorkerResponse["encrypted_out_bias_id"]
        predict1_encrypted_out_predictions_id   = jsonWorkerResponse["encrypted_out_predictions_id"]

        logger.debug({
            "predict1_encrypted_out_weights_id"     : predict1_encrypted_out_weights_id,
            "predict1_encrypted_out_bias_id"        : predict1_encrypted_out_bias_id,
            "predict1_encrypted_out_predictions_id" : predict1_encrypted_out_predictions_id, 
        })
