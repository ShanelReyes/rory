...
from rory.core.validationindex.metrics import internal_validation_indexes,external_validation_indexes
# 
...
def skmeans():
    try:
        ...
        encrypted_matrix_put_response = STORAGE_CLIENT.put_ndarray(key = encryptedMatrixId, ndarray = outsourced.encrypted_matrix).unwrap() # The encrypted matrix is placed in the storage system
        UDM_put_response              = STORAGE_CLIENT.put_ndarray(key = UDMId, ndarray = outsourced.UDM) # The udm array is placed in the storage system
        ...
        while (status != Constants.ClusteringStatus.COMPLETED): #While the status is not completed
            ...
            encryptedShiftMatrix_get_response = STORAGE_CLIENT.get_ndarray(key = encryptedShiftMatrixId, sink_path = SINK_PATH, delete = False).unwrap()
            # if (encryptedShiftMatrix_get_result.is_ok):
            # encryptedShiftMatrix_response = encryptedShiftMatrix_get_result.unwrap()
            ...
            shiftmatrix_put_response = STORAGE_CLIENT.put_ndarray(key = shiftMatrixId, ndarray = np.array(shiftMatrix.matrix)).unwrap() #Shift matrix is saved to the storage system
            # if(shiftmatrix_put_result.is_ok):
            # shiftmatrix_response = shiftmatrix_put_result.unwrap()
...
def kmeans():
    try:
        ...
        plaintextMatrix_path       = "{}/{}.{}".format(SOURCE_PATH, plainTextMatrixId, extension)
        plaintextMatrix            = pd.read_csv(plaintextMatrix_path, header=None).values
        plaintextMatrix_put_response = STORAGE_CLIENT.put_ndarray(key = plainTextMatrixId, ndarray = plaintextMatrix).unwrap()
        managerResponse:SecureClusteringManager = current_app.config.get("manager") # Communicates with the manager
        mr = managerResponse.getWorker( #Gets the worker from the manager
            headers = {
                "Algorithm": algorithm,
        ...
        return Response(response = None, status= 500, headers = {"Error-Message":str(e)})
    # else:
        # error = plaintextMatrix_put_result.unwrap_err()
        # return Response(response = None, status= 500, headers={"Error-Message":error})  
...
def dbskmeans():
    try:
        ...
        _                 = STORAGE_CLIENT.put_ndarray(key = encryptedMatrixId, ndarray = outsourced.encrypted_matrix).unwrap() # The encrypted matrix is placed in the storage system
        _                 = STORAGE_CLIENT.put_ndarray(key = UDMId, ndarray = outsourced.UDM).unwrap() # The udm array is placed in the storage system
        managerResponse:SecureClusteringManager = current_app.config.get("manager") # Communicates with the manager
        get_worker_start_time = time.time()
        mr = managerResponse.getWorker( #Gets the worker from the manager
            headers = {
        ...
        logger.debug("Response {}".format(jsonResponse))
        ...
        while (status   != Constants.ClusteringStatus.COMPLETED): #While the status is not completed
            ...
            logger.debug("Response {}".format(jsonWorkerResponse))
            ...
            _                      = STORAGE_CLIENT.put_ndarray(key = shiftMatrixId, ndarray = np.array(shiftMatrix.matrix)).unwrap() #Shift matrix is saved to the storage system
            _                      = STORAGE_CLIENT.put_ndarray(key = shiftMatrixOpeId, ndarray = np.array(shiftMatrixOpe.matrix)).unwrap() #Shift matrix is saved to the storage system
            status                 = Constants.ClusteringStatus.WORK_IN_PROGRESS #Status is updated
            run2_headers           = {
                    "Step-Index":"2",
                    "Clustering-Status": str(status),
